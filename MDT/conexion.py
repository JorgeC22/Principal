...
class dbMDT():
    ...
    def insertaRegistro(self, **params):   
        try:
            ...
            self.cur.execute("""
                INSERT INTO mdt (Clave_de_Trx, Tipo_de_Trx, Fecha_Origen, Fecha_Posteo, Hora, Banco_Emisor, Banco_Adquirente, Numero_de_Cuenta, 
                        ...
                        Tkn_B4_S3, Tkn_B4_S4, Tkn_B4_S5, Tkn_B4_S6, Tkn_B4_S7, Tkn_B4_S8, Tkn_B4_S9, Tkn_B5_S1, Tkn_B5_S2, Tkn_B5_S3, Tkn_B5_S4, Tkn_B5_S5)
                        VALUES
                        (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
                        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
    ...
    def existeRegistro(self, i):
        try:
            self.cur = self.conn.cursor(dictionary=True, prepared=True)
            self.cur.execute("""
                SELECT COUNT(*) as cuenta FROM mdt WHERE
                    Fecha_Origen = ?
                    and Numero_de_Cuenta = ?
                    and Monto = ?
                    and Numero_de_Referencia = ?;
            """, ([i[2],i[7],i[10],i[11]]))
